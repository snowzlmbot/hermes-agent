...
final class StaticContractTests: XCTestCase {
    ...
    func testContractFixtureIsBundledForRuntimeCapabilityChecks() throws {
        ...
        XCTAssertEqual(contract.sessionLibrary.scope, "loaded_sessions_only")
        XCTAssertEqual(contract.sessionLibrary.searchFields, ["display_title", "title", "preview", "stored_session_id"])
        XCTAssertEqual(contract.sessionLibrary.queryNormalization, "trim_case_insensitive")
        XCTAssertTrue(contract.sessionLibrary.preserveOrder)
        XCTAssertTrue(contract.sessionLibrary.preservePinnedOrder)
        XCTAssertTrue(contract.sessionLibrary.activeArchivedViews)
        XCTAssertEqual(contract.sessionLibrary.restoreAction, "set_archived_false_without_selecting")
        XCTAssertEqual(contract.sessionLibrary.noMatchAction, "show_empty_state")
        XCTAssertEqual(contract.sessionLibrary.clearQueryAction, "show_current_view")
        XCTAssertFalse(contract.sessionLibrary.serverSearchOrPagination)
        XCTAssertTrue(contract.sessionLibrary.profileScoped)
        ...
        XCTAssertEqual(contract.eventReplayProtection.currentGatewayEventIdentity, "not_uniformly_available")
        XCTAssertFalse(contract.eventReplayProtection.currentGatewayReplayCursor)
        XCTAssertEqual(contract.eventReplayProtection.safeDeduplicationScope, "stable_identity_only")
        XCTAssertFalse(contract.eventReplayProtection.exactlyOnceGuarantee)
